...
struct SA {
    long long st[22][N], log[N];
    long long sa[N], sa_[N], rk[N], rk_[N], cnt[N];
    long long n;
    ...
    void init(std::string s, long long len) {
        ...
        for (long long i = 2; i <= n; i ++) log[i] = log[i >> 1] + 1;
        long long m = 300;
        for (long long i = 1; i <= n; i ++) cnt[s[i]] ++;
        for (long long i = 1; i <= m; i ++) cnt[i] += cnt[i - 1];
        for (long long i = n; i; i --) sa[cnt[s[i]] --] = i;
        for (long long i = 1; i <= n; i ++) rk[sa[i]] = rk[sa[i - 1]] + (s[sa[i]] != s[sa[i - 1]]);
        for (long long i = 1; i <= n; i ++) st[0][i] = rk[i];
        for (long long i = 0; i <= m; i ++) cnt[i] = 0;
        m = rk[sa[n]];
        for (long long k = 1; k < n; k <<= 1) {
            long long p = 0;
            for (long long i = n - k + 1; i <= n; i ++) sa_[++ p] = i;
            for (long long i = 1; i <= n; i ++) 
                if (sa[i] > k) sa_[++ p] = sa[i] - k; 
                // sa[i] 是第二关键字的下标，那么 sa[i] - k 就是第一关键字的下标
            for (long long i = 1; i <= n; i ++) cnt[rk[sa_[i]]] ++;
            for (long long i = 1; i <= m; i ++) cnt[i] += cnt[i - 1];
            for (long long i = n; i; i --) sa[cnt[rk[sa_[i]]] --] = sa_[i];
            for (long long i = 0; i <= m; i ++) cnt[i] = 0;
            for (long long i = 1; i <= n; i ++) {
                rk_[sa[i]] = rk_[sa[i - 1]] + cmp_sa(sa[i - 1], sa[i], k);
            }
            for (long long i = 1; i <= n; i ++) rk[i] = rk_[i], st[log[k] + 1][i] = rk[i];
            m = rk[sa[n]];
        }
    }
